...
from .. import ui
from ..core import activity, domains, mailbox, names
...
def status(target: Target = None) -> None:
    ...
    For a domain: whether its DNS records and the certificate are right, and its addresses, forwards and disk usage.
...
def _domain_status(session: Session, domain: str) -> None:
    ui.heading(domain)
    _section("DNS records and certificate", lambda: _checks(session, domain))
...
def _checks(session: Session, domain: str) -> None:
    facts = checks.server(session)
    with ui.console.status("Checking the DNS records…"):
        found = checks.domain_checks(session, facts, domain)
    checks.show(found)
