...
from .dns import DnsRecord, Status
...
def error(problem: CtlError) -> None:
...
def ask(prompt: str, value: str | None, argument: str) -> str:
    ...
    if not interactive():
        raise CtlError(
            f"{argument} is missing.",
            hint="Give it on the command line, or run the command in a terminal to be asked for it.",
        )
    answer = ""
    while not answer:
...
def _no_terminal(question: str, flags: str) -> CtlError:
    return CtlError(f"Can't ask without a terminal: {question}", hint=f"Add {flags}.")


def _cancelled() -> CtlError:
    return CtlError("Cancelled. Nothing was changed.")
