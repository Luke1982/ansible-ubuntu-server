...
The config is set up by hand in WebAdmin, so a tool only adds and removes its own virtual hosts, template members
and the listeners' names for them, in the layout WebAdmin writes, and leaves everything else as it is. It doesn't
use 'include': a config file holding an include line is read-only in WebAdmin, so every change is made in
httpd_config.conf itself.

A change is made by reading the config, editing the lines and renaming a new file into place, so two tools doing
that at the same time would lose one another's changes: the second writes what it read before the first saved.
Everything that changes the config must therefore run inside locked(), over the whole of the read, the edit and
the write: locking only the write would let the loser read before the winner saved. Reading without changing
needs no lock.

The lock only binds the tools that take it, and they only shut each other out while they use the same LOCK_FILE.
mailctl takes it too, from its own copy of this module in roles/mail, so the value below must not be changed in
one copy alone. That is also why no tool has a setting for it.
"""
...
# Every tool that changes OpenLiteSpeed's config waits for this one lock, so the path must be the same in all of
# them. It is outside OpenLiteSpeed's config directory, which belongs to WebAdmin's user, and on a tmpfs, so a
# lock never outlives the reboot that killed whatever held it.
...
def locked(path: Path | None = None, timeout: float = LOCK_TIMEOUT) -> Iterator[None]:
    ...
    so the tests can put the lock somewhere a normal user may write. No tool has a setting for it: two tools
    with different paths would take different locks and shut nobody out.
    """
    ...
    while True:
        try:
            ...
            if error.errno not in (errno.EACCES, errno.EAGAIN):
                raise CtlError(f"Can't lock {path}: {error.strerror}.") from None
...
def write(root: Path, lines: list[str], tool: str = "serverctl") -> None:
    """Replaces the config in one step, with the same owner and mode, and keeps the previous version next to it as
    httpd_config.conf.TOOL.bak.

    The config directory belongs to WebAdmin's user and these tools run as root, so nothing here follows a symbolic
    ...
    _replace(path.with_name(f"{path.name}.{tool}.bak"), previous, status)
    _replace(path, "\n".join(lines) + "\n", status)
...
def _read(path: Path) -> tuple[str, os.stat_result]:
    try:
        ...
        raise CtlError(f"There is no OpenLiteSpeed config at {path}.",
                           hint="The web role of the Ansible playbook installs OpenLiteSpeed.") from None
    except OSError as error:
        if error.errno == errno.ELOOP:
            raise CtlError(f"{path} is a symbolic link, which isn't followed.") from None
        raise CtlError(f"Can't read {path}: {error.strerror}.") from None
...
def restart(root: Path) -> None:
    ...
    if "[ERROR]" in output:  # lswsctrl reports some failures only in its output
        raise CtlError(f"lswsctrl restart failed: {output}")
...
def member_details(lines: list[str], template: str) -> list[Member]:
    """The template's members with the domain and aliases each of them serves."""
    found = _template(lines, template)
    if not found:
        return []
    details = []
    for block in blocks(lines):
        if block.kind != "member" or not found.start < block.start < found.end:
            continue
        aliases = value(lines, block, "vhAliases") or ""
        details.append(Member(block.name, value(lines, block, "vhDomain") or "",
                              tuple(alias for alias in re.split(r"[,\s]+", aliases) if alias)))
    return details
...
def with_member(lines: list[str], member: Member, template: Template, others: tuple[Template, ...] = ()) -> list[str]:
    ...
    lines = with_template(lines, template)
...
def with_template(lines: list[str], template: Template) -> list[str]:
    """The config with the template block, added at the end or with its file, listeners and note brought up to
    date. Its members are left as they are."""
...
def _unreadable(reason: str) -> CtlError:
    return CtlError(f"OpenLiteSpeed's config can't be read: {reason}.",
                    hint="Only the layout WebAdmin writes is understood: a block's { at the end of its first "
                         "line, and its } on a line of its own.")
