...
def run_starter(*args: str) -> str:
    ...
    with tempfile.TemporaryFile("w+", errors="replace") as output:
        try:
            ...
            raise CtlError(f"{args[0]} isn't installed.") from None
        output.seek(0)
        text = output.read().strip()
    if result.returncode != 0:
        raise CtlError(f"{shlex.join(args)} failed: {text or f'exit status {result.returncode}'}")
    return text
...
def _run(args: tuple[str, ...], stdin, **text_options):
    ...
    if result.returncode != 0:
        ...
        raise CtlError(f"{shlex.join(args)} failed: {detail}")
...
def require_root(tool: str) -> None:
    if os.geteuid() != 0:
        raise CtlError(f"{tool} must run as root.", hint="Try again with sudo.")
...
def find_user(name: str) -> pwd.struct_passwd:
    try:
        ...
        raise CtlError(f"There's no user {name} on this server.") from None
...
def remove_tree(path: Path) -> None:
    ...
    if path.is_symlink():
        raise CtlError(f"Can't delete {path}: it's a symbolic link.")
    try:
        shutil.rmtree(path)
    except FileNotFoundError:
        pass
    except OSError as error:
        raise CtlError(f"Can't delete {path}: {error.strerror or error}.") from None
...
def server_ips() -> set[IPv4Address | IPv6Address]:
    """The addresses this server is reached on: the source addresses the kernel picks for its default IPv4 and
    IPv6 routes. Other addresses, like failover addresses or extra IPv6 addresses, aren't."""
    ips: set[IPv4Address | IPv6Address] = set()
    problem = None
    for probe in _ROUTE_PROBES:
        try:
            ips |= parse_route_sources(run("ip", "-json", "route", "get", probe))
        except CtlError as error:  # usually no route for that kind of address
            problem = error
    if not ips:
        reason = problem.message if problem else "its default routes have no source address."
        raise CtlError(f"Can't tell which addresses this server uses: {reason}")
    return ips
